	...
	: describe("FundMe", async () => {
			...
			describe("receive", async () => {
				it("Should be able to receive ether", async () => {
					const startingBalance = await ethers.provider.getBalance(fundMe);
					const signerDeployer = await ethers.provider.getSigner(deployer);
					const txResponse = await signerDeployer.sendTransaction({ to: fundMe, value: sendValue });
					const txReceipt = await txResponse.wait(1);
					const { gasUsed, gasPrice } = txReceipt;
					const txCost = gasUsed * gasPrice;

					const endingBalance = await ethers.provider.getBalance(fundMe);
					assert.equal((startingBalance + sendValue).toString(), endingBalance.toString(), "receive ether failed");
				});
			});
		});
